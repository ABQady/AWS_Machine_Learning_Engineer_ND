...
def test(model, test_loader, criterion):
    logger.debug(f"Testing model on entire testing dataset")
    model.eval()
    running_loss=0
    running_corrects=0
...
def train(model, train_loader, validation_loader, criterion, optimizer):
    ...
    for epoch in range(epochs):
        ...
        if loss_counter==1:
            logger.info(f"Breaking because increasing loss at epoch no:  {epoch}")
            break
        if epoch==0:
            logger.info(f"Breaking because epoch limit at epoch no:  {epoch}")
            break
    return model
